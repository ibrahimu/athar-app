...
enum Zakat {
    /// نصاب الذهب عشرون مثقالًا ≈ 85 غرامًا، ونصاب الفضة مئتا درهم ≈ 595 غرامًا.
    ...
    static func compute(_ i: ZakatInput) -> ZakatResult {
        ...
        // نصابان لا واحد: نصاب الذهب (85غ) ونصاب الفضة (595غ). والفقهاء المعاصرون
    ...
    ///
    /// آيةُ الزكاة تُحلّ من quran.json بمرجعها ولا تُكتب هنا: كانت مكتوبةً برسمٍ إملائي
    /// (الصلاة والزكاة) يخالف عثمانيَّ المصحف (ٱلصَّلَوٰةَ وٱلزَّكَوٰةَ)، ومقطوعةً دون تمامها.
    /// والحديثان بعدها لا مدخل لهما في hadith.json، فبقي لفظهما وعزوهما كما كُتبا —
    /// وإدخالُ روايةٍ إلى بيانات التطبيق قرارُ صاحبه بمصدرٍ يعتمده لا قرارُ هذا الملف.
    static let evidence: [(text: String, source: String)] = {
        var out: [(text: String, source: String)] = []
        let ref = AyahRef(surah: 2, ayah: 43)
        if let text = Quran.text(ref), let surah = Quran.surah(ref.surah) {
            out.append((text, "سورة \(surah.name): \(ref.ayah.counterText)"))
        }
        out.append(("وفي الرِّقَة ربع العشر.", "رواه البخاري"))
        out.append(("ليس فيما دون خمس أواقٍ صدقة.", "متفق عليه"))
        return out
    }()

    static let notes: [String] = [
        "تجب الزكاة إذا بلغ المال النصاب ومضى عليه حَوْلٌ هجري كامل.",
        "النصاب: 85 غرامًا ذهبًا أو 595 غرامًا فضة. ويُحسب هنا بالأدنى قيمةً منهما مما أدخلتَ سعره، وهو الأحظّ للفقراء.",
