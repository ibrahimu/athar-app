...
enum Reminders {
    ...
    private static func scheduleWird(store: AtharStore) {
        ...
        add(id: wirdId,
                  ...
                  // ولفظُه ليس في `hadith.json` (رياض الصالحين والأربعين)، لكنّ عزوه
                  // لا يُكتب من الذاكرة: رُوجع في الموسوعة الحديثية (الدرر السنية)
                  // وموسوعة الأحاديث النبوية — متّفقٌ عليه من حديث عائشة رضي الله
                  // عنها (البخاري ٦٤٦٥، مسلم ٧٨٣). فيُعزى كما يُعزى في شاشة الورد.
                  body: loc("%1$@ تكفيك اليوم — «أحبُّ الأعمال إلى الله أدومها» متفق عليه.", store.wirdTarget.ayahCountText),
    ...
    private static func scheduleQiyam(store: AtharStore) {
        ...
        for day in 0..<7 {
            ...
            // معرّف يُقرأ منه تخريجه. وعزوٌ يُكتب من الذاكرة أسوأ من لا عزو — فرُوجع
            // في الموسوعة الحديثية (الدرر السنية) وموسوعة الأحاديث النبوية: متّفقٌ
            // عليه من حديث أبي هريرة رضي الله عنه. وهو العزو نفسه الذي تعرضه شاشة
            // الصلاة لهذا المتن، فاستوى ما يُقرأ في الشاشة وما يصل في التنبيه.
            content.body = loc("«ينزل ربنا إلى السماء الدنيا حين يبقى ثلث الليل الآخر فيقول: من يدعوني فأستجيب له» متفق عليه")
            content.sound = .default
